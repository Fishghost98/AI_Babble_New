...
void PCF85063_Init()
{
	...
	Now_datetime.year = 2026;
	Now_datetime.month = 1;
	Now_datetime.day = 18;
	Now_datetime.dotw = 0;
	Now_datetime.hour = 9;
	Now_datetime.minute = 3;
...
void PCF85063_Read_Time(datetime_t *time)
{
	...
	if(time->month < 10 && time->day < 10){
		sprintf(time->date_str, "0%d / 0%d", time->month, time->day);
	}else if(time->month < 10){
		sprintf(time->date_str, "0%d / %d", time->month, time->day);
	}else if(time->day < 10){
		sprintf(time->date_str, "%d / 0%d", time->month, time->day);
	}

	switch (time->dotw)
	{
	case 1:
		strcpy(time->week_day, "星期一");
		break;
	case 2:
		strcpy(time->week_day, "星期二");
		break;
	case 3:
		strcpy(time->week_day, "星期三");
		break;
	case 4:
		strcpy(time->week_day, "星期四");
		break;
	case 5:
		strcpy(time->week_day, "星期五");
		break;
	case 6:
		strcpy(time->week_day, "星期六");
		break;
	case 0:
		strcpy(time->week_day, "星期日");
		break;
	default:
		break;
	}
