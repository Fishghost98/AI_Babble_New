...
private:
    ...
    void Initializest77916Display() {
        ...
        if (esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)QSPI_LCD_HOST, &io_config,
                                     &panel_io) != ESP_OK) {
    ...
    /**
     * @brief  触摸中断回调函数（实时重置睡眠定时器）
     */
    static void cst816_touch_interrupt_cb(esp_lcd_touch_handle_t tp) {
        // 检测到触摸操作，记录当前操作时间
        // all_config.page_systemConfig.last_operation_time = millis();
        esp_rom_printf("Touch interrupt\n");
    }
    ...
    void InitializeTouch() {
        ...
        esp_lcd_touch_config_t tp_cfg = {
            ...
            .interrupt_callback = cst816_touch_interrupt_cb,
        };
